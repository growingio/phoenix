 ...
 * @since 0.1
 */
@BuiltInFunction(name = CBitMapMergeFunction.NAME, nodeClass = CBitMapMergeAggregateParseNode.class,
        args = {@Argument(allowedTypes = {PVarbinary.class})})
public class CBitMapMergeFunction extends DelegateConstantToCountAggregateFunction {
    public static final String NAME = "CBITMAP_MERGE";

    public CBitMapMergeFunction() {
    }

    // TODO: remove when not required at built-in func register time
    public CBitMapMergeFunction(List<Expression> childExpressions) {
    ...
    private Aggregator newAggregator(final PDataType type, SortOrder sortOrder,
        ...
        return new CBitMapMergeAggregator(sortOrder, ptr);
    }
